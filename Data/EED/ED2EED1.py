...
def main():
    dialogue_train = np.load("EED/data/ED/sys_dialog_texts.train.npy", allow_pickle=True)
    situation_train = np.load("EED/data/ED/sys_situation_texts.train.npy", allow_pickle=True)
    emotion_train = np.load("EED/data/ED/sys_emotion_texts.train.npy", allow_pickle=True)
    target_train = np.load("EED/data/ED/sys_target_texts.train.npy", allow_pickle=True)
    len_train = len(dialogue_train)
    json_path = "EED/EED04.json"
    copy_path = "EED/EED04_copy.json"
    ...
    volume = 8000
    for i in tqdm(range(6000+len(EED_dataset), volume), desc="Task Progress"):
